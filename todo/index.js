...
app.delete("/delete/:id", (req, res)=>{
    ...
    let todoss = initialTodo.filter((todos) => todos.id != id);

    res.status(200).send({deletedTodo : deletetodo, todos:todoss});
...
app.get("/findbystatus", (req, res)=>{
    ...
    let filters = initialTodo.filter((todos) => todos.isCompleted.toString() == isCompleted);
    res.status(200).send(filters);
})

const port = 8090;
app.listen(port, () => {
  console.log(`Server is runnig on port : http://localh